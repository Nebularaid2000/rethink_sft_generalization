...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    if "format_reward" in batch.non_tensor_batch:
        print("type of format_reward:", type(batch.non_tensor_batch["format_reward"])) # ndarray
        print("len(format_reward):", len(batch.non_tensor_batch["format_reward"]))
        print("format_reward:", batch.non_tensor_batch["format_reward"])
        format_rewards = torch.tensor(batch.non_tensor_batch["format_reward"], device=advantages.device, dtype=torch.float32)
        metrics["critic/format_reward/mean"] = torch.mean(format_rewards).detach().item()
        metrics["critic/format_reward/max"] = torch.max(format_rewards).detach().item()
        metrics["critic/format_reward/min"] = torch.min(format_rewards).detach().item()
