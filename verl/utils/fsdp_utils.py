...
def apply_fsdp2(model, fsdp_kwargs, config):
    ...
    try:
        ...
        if not default_transformer_cls_names_to_wrap and isinstance(model, Mistral3ForConditionalGeneration):
            print("setting Mistral3ForConditionalGeneration default_transformer_cls_names_to_wrap")
            default_transformer_cls_names_to_wrap = [
                "Ministral3DecoderLayer",
                "PixtralAttentionLayer",
            ]
    except ImportError:
        print("error in importing Mistral3ForConditionalGeneration")
    ...
    print(f"fsdp_transformer_layer_cls_to_wrap: {fsdp_transformer_layer_cls_to_wrap}")

    assert len(fsdp_transformer_layer_cls_to_wrap) > 0 and fsdp_transformer_layer_cls_to_wrap[0] is not None, \
        f"len(fsdp_transformer_layer_cls_to_wrap) = {len(fsdp_transformer_layer_cls_to_wrap)}, " \
