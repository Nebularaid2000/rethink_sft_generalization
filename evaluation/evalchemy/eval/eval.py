...
def evaluate(
    ...
    **eval_kwargs,
    ...
    verbosity_level = verbosity or 'INFO'
    ...
    if benchmark_tasks:
        ...
        generate_methods = task_manager.get_list_generate_responses(benchmark_tasks) # 获取对应benchmark的generate_responses(位于eval_instruct.py)
        generation_results = []
        ...
        if lm is not None and not hasattr(lm, "upload_to_hub"):
            evaluate_methods = task_manager.get_list_evaluates(valid_tasks) # 获取对应benchmark的evaluate_responses(位于eval_instruct.py)
...
def cli_evaluate(args: Optional[argparse.Namespace] = None) -> None:
    ...
    eval_logger = logging.getLogger(__name__)
    verbosity_level = getattr(args, 'verbosity', 'INFO') or 'INFO'
    eval_logger.setLevel(getattr(logging, verbosity_level))
